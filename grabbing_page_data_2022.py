...
for team in myresult:
    ...
    values = (data1,data2)
    mycursor = mydb.cursor()
    sql = "SELECT `ACTUAL OVER UNDER RESULT:` FROM `previous_losses_mlb_v5_with_date_agg` WHERE `HOME TEAM:` = %s OR `AWAY TEAM:` = %s"
    sql2 = "SELECT `Win Or Lose:` FROM `previous_losses_mlb_v5_with_date_agg` WHERE `HOME TEAM:` = %s OR `AWAY TEAM:` = %s"
    sql3 = "SELECT `Win Or Lose:` FROM `previous_losses_mlb_v5_with_date_agg` WHERE `HOME TEAM:` = %s OR `HOME TEAM:` = %s"
    sql4 = "SELECT `Win Or Lose:` FROM `previous_losses_mlb_v5_with_date_agg` WHERE `AWAY TEAM:` = %s OR `AWAY TEAM:` = %s"
    sql00 = "SELECT `Win Or Lose:` FROM previous_losses_mlb_v5_with_date_agg WHERE weekday(`DATE:`) = '0' AND (`HOME TEAM:` = %s OR `AWAY TEAM:` = %s)"
    sql000 = "SELECT `Win Or Lose:` FROM previous_losses_mlb_v5_with_date_agg WHERE weekday(`DATE:`) = '1' AND (`HOME TEAM:` = %s OR `AWAY TEAM:` = %s)"
    sql01 = "SELECT `Win Or Lose:` FROM previous_losses_mlb_v5_with_date_agg WHERE weekday(`DATE:`) = '1' AND (`HOME TEAM:` = %s OR `AWAY TEAM:` = %s)"
    sql02 = "SELECT `Win Or Lose:` FROM previous_losses_mlb_v5_with_date_agg WHERE weekday(`DATE:`) = '2' AND (`HOME TEAM:` = %s OR `AWAY TEAM:` = %s)"
    sql03 = "SELECT `Win Or Lose:` FROM previous_losses_mlb_v5_with_date_agg WHERE weekday(`DATE:`) = '3' AND (`HOME TEAM:` = %s OR `AWAY TEAM:` = %s)"
    sql04 = "SELECT `Win Or Lose:` FROM previous_losses_mlb_v5_with_date_agg WHERE weekday(`DATE:`) = '4' AND (`HOME TEAM:` = %s OR `AWAY TEAM:` = %s)"
    sql05 = "SELECT `Win Or Lose:` FROM previous_losses_mlb_v5_with_date_agg WHERE weekday(`DATE:`) = '5' AND (`HOME TEAM:` = %s OR `AWAY TEAM:` = %s)"
    sql06 = "SELECT `Win Or Lose:` FROM previous_losses_mlb_v5_with_date_agg WHERE weekday(`DATE:`) = '6' AND (`HOME TEAM:` = %s OR `AWAY TEAM:` = %s)"
    ...
    print("WHAT THE FUCK")
    sql5 = "SELECT `HOME TEAM POINTS:` FROM `previous_losses_mlb_v5_with_date_agg` WHERE `HOME TEAM:` = %s OR `HOME TEAM:` = %s"
    mycursor.execute(sql5, values)
    myresult = mycursor.fetchall()
    home_home_points.append(myresult[-5:])
    sql6 = "SELECT `AWAY TEAM POINTS:` FROM `previous_losses_mlb_v5_with_date_agg` WHERE `AWAY TEAM:` = %s OR `AWAY TEAM:` = %s"
    ...
    sql7 = "SELECT `ACTUAL OVER UNDER RESULT:` FROM `previous_losses_mlb_v5_with_date_agg` WHERE `HOME TEAM:` = %s OR `HOME TEAM:` = %s"
    mycursor.execute(sql7, values)
    myresult = mycursor.fetchall()
    home_home_overunder.append(myresult)
    sql8 = "SELECT `ACTUAL OVER UNDER RESULT:` FROM `previous_losses_mlb_v5_with_date_agg` WHERE `AWAY TEAM:` = %s OR `AWAY TEAM:` = %s"
...
for element1, element2, element3, element4 in zip_object:
    ...
    sql5 = "SELECT `ACTUAL OVER UNDER RESULT:` FROM `previous_losses_mlb_v6_with_date_agg` WHERE `HOME TEAM:` = %s AND `AWAY TEAM:` = %s OR `HOME TEAM:` = %s AND `AWAY TEAM:` = %s"
...
stats_doc = ExcelWriter(yesterday.strftime('Current_Day_Over_Under_2022.xlsx'), engine='openpyxl')
dataframe_schedule.to_excel(stats_doc, sheet_name='Sheet1', index=False)
stats_doc.close()

excel_data_df4 = pd.read_excel(yesterday.strftime('Current_Day_Over_Under_2022.xlsx'), sheet_name='Sheet1', engine='openpyxl')
excel_data_df4.to_sql(con=database_connection, name='current_day_statsmlbv5_2022', if_exists='replace', index=False)
