...
page = requests.get('https://www.cbssports.com/nhl/stats/team/opponent/penalties/nhl/regular/')
...
dataframe_team_total_points_total_games = pd.DataFrame({'TEAM:': team_list, 'GAMES PLAYED:': games_play, 'OPP TOTAL PENALTY MINS:': total_penalty_mins})

print(dataframe_team_total_points_total_games)

stats_doc = ExcelWriter('NHL_OPP_PENALTY_STATS.xlsx')
dataframe_team_total_points_total_games.to_excel(stats_doc, sheet_name='Sheet1', index=False)
stats_doc.close()


excel_data_df2 = pd.read_excel('NHL_OPP_PENALTY_STATS.xlsx', sheet_name='Sheet1', engine='openpyxl')
...
stats_doc = ExcelWriter('NHL_OPP_PENALTY_STATS.xlsx', engine='openpyxl')
